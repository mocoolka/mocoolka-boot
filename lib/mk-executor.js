...
var debug = require('debug')('mocoolka:boot:executor');
...
module.exports = function compileOptions(app, instructions, actions, callback) {
  ...
  actions.forEach(function(action) {
    //noinspection Eslint,Eslint,Eslint,Eslint,Eslint
    if (action === 'Config') {
    ...
    } else { //noinspection Eslint,Eslint,Eslint
      if (action === 'Datasources') {
        ...
        throw new Error(
          g.f('The "%s"' +
            ' is in the unsupported format.Value must is ' +
            'in [Config,Models,Datasources,Middleware,' +
            'Components,Mixins,BootScripts]', action));
      }
    }
  });
};
//noinspection Eslint
function executeModel(app, instructions, callback) {
  callback = callback || function() {};
...
function setupDataSources(app, instructions) {
  forEachKeyedObject(instructions.dataSources, function(key, obj) {
    ...
    if (lazyConnect) {
      obj.lazyConnect = !(lazyConnect === 'false' || lazyConnect === '0');
    }
  ...
  function getConfigVariable(param) {
    ...
    if (match) {
      ...
      if (useEnvVars && process.env[varName] !== undefined) {
        ...
        configVariable = app.get(varName);
      } else {
