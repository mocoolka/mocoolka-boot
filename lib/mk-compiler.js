...
var ConfigLoader = require('./mk-config-loader');
var utils = require('./utils');
var debug = require('debug')('mocoolka:boot:compiler');
...
module.exports = function compileOptions(options, actions) {
  options = options || {};
  ...
  instructions.env = env;

  assert.isArray(actions, 'complieOptions second param must is Array');
  actions.forEach(function(action) {
    if (action === 'Config') {
      instructions.config = compileConfig(options, appRootDir, env);
    } else if (action === 'Models') {
      compileModel(options, appRootDir, env, instructions);
    } else if (action === 'Datasources') {
      instructions.dataSources = compileDatasources(options, appRootDir, env);
    } else if (action === 'Middleware') {
      instructions.middleware = compileMiddleware(options, appRootDir, env);
    } else if (action === 'Components') {
      instructions.components = compileComponents(options, appRootDir, env);
    } else if (action === 'BootScripts') {
      instructions.files = {};
      instructions.files.boot = compileBootScripts(options, appRootDir, env);
    } else {
      throw new Error(
        g.f('The "%s"' +
          ' is in the unsupported format.Value must is in' +
...
function compileConfig(options, appRootDir, env) {
...
function compileModel(options, appRootDir, env, instructions) {
  ...
  buildAllModelInstructions(
    modelsRootDir, modelsConfig, modelSources,
    options.modelDefinitions, instructions);

  var mixinDirs = options.mixinDirs || [];
  var mixinSources = options.mixinSources || modelsMeta.mixins || ['./mixins'];
  instructions.mixinInstructions = buildAllMixinInstructions(
    appRootDir, mixinDirs, mixinSources, options, instructions.models);
}
function compileDatasources(options, appRootDir, env) {
...
function compileMiddleware(options, appRootDir, env) {
  ...
  return buildMiddlewareInstructions(middlewareRootDir, middlewareConfig);
}

function compileComponents(options, appRootDir, env) {
  ...
  return buildComponentInstructions(componentRootDir, componentConfig);
}

function compileMixins(options, appRootDir, env, modelInstructions) {
  var modelsRootDir = options.modelsRootDir || appRootDir;
...
//noinspection Eslint
function compileBootScripts(options, appRootDir, env) {
  // require directories
...
function mkbuildAllModelInstructions(rootDir, modelsConfig, sources,
  modelDefinitions, instructions) {
...
function buildAllModelInstructions(rootDir, modelsConfig, sources,
  ...
  var registry = verifyModelDefinitions(rootDir, modelDefinitions) ||
    findModelDefinitions(rootDir, sources);
  for (var pro in registry) {
    var model = registry[pro];

    if (model.definition && model.definition.mkautobuild === true) {
      if (!instructions.dataSources[model.definition.name + 'DB']) {
        instructions.dataSources[model.definition.name + 'DB'] = {
          'name': model.definition.name + 'DB',
          'connector': 'memory',
          'file': './store/' + model.definition.name + '.json'
        };
      }
      if (!modelsConfig[model.definition.name]) {
        modelsConfig[model.definition.name] = {
          'dataSource': model.definition.name + 'DB',
          'public': true
        };
      }
    }
  ...
  var modelInstructions = modelNamesToBuild
    .map(function createModelInstructions(name) {
      var config = modelsConfig[name];
      var definition = registry[name] || {};
  ...
  instructions.models = sortByInheritance(modelInstructions);
}
  ...
  fullPath = modulePaths
    .map(function(candidateDir) {
      ...
      try {
        ...
        var filePath = String(require.resolve(absPath));
