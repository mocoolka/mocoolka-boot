...
'use strict';
/**
 * A Module that initialize an application as mocoolka server
 * @module mocoolka/boot */
var ConfigLoader = require('./lib/mk-config-loader');
var compile = require('./lib/mk-compiler');
var execute = require('./lib/mk-executor');
var addInstructionsToBrowserify = require('./lib/mk-bundler');
var utils = require('./lib/utils');


/**
 * InitObject used in initialize an application.
 ...
 *
 * @param {Object} app - Mocoolka application created by `mocoolka()`.
 * @param {String|module:mocoolka/boot~InitObject} [options] - Boot options; If String, this is
 * the application root directory; if object, has below properties.
 * @see [initObject]{@link module:mocoolka/boot~InitObject}
 * @param {string[]} [actions] - List of action to init .
 * @param callback - Callback function.
 * @summary Initialize an application
 */
exports.bootMocoolkaApp = function(app, options, actions, callback) {
  // backwards compatibility with loopback's app.boot
  if (typeof options === 'object') {
    options.env = options.env || app.get('env');
  }

  var instructions = compile(options, actions);
  execute(app, instructions, actions, callback);
};

/**
 * Compile boot instructions and add them to a browserify bundler.
 * @param {Object|InitObject} options as described in `bootLoopBackApp` above.
